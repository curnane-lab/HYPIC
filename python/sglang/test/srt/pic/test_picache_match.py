...
def test_match_no_segments_returns_empty():
    ...
    assert r.pic_segment_entries is None or all(x is None for x in r.pic_segment_entries)
...
def test_match_some_hit_some_miss():
    ...
    entry = pc._insert_segment(h, seg_ids, torch.tensor([900,901,902], dtype=torch.int64), 7)

    req = _Req(
        pic_segments=[(0,3), (3,6), (6,9)],
        fill_ids=[1,2,3, 10,11,12, 99,98,97],
    )
    r = pc.match_prefix(MatchPrefixParams(key=RadixKey(token_ids=req.fill_ids), req=req))
...
def test_match_refreshes_last_access_time():
    ...
    req2 = _Req(pic_segments=[(0,3), (3,6), (6,7)], fill_ids=[1,2,3, 10,11,12, 5])
